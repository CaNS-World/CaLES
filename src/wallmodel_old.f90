...
module mod_wallmodel
  ...
  subroutine cmpt_wallmodelbc(n,ibound,idir,nh,mtype,l,dl,zc,zf,dzc,dzf,visc,h,index,vel1,vel2, &
    ...
    case(3)
      ...
      !
      ! must do exchange in this file, but how to do it?   uh vh wh   du/dy dv/dy dw/dy   plane points
      ! call exchange_data(u,v,w,bcu,bcv,bcw)
      ! then in exchange_data_wall_model, compute features and send states and receive actions
      ! use bcu(i,j,ibound) = sgn*visci*tauw(1)
      !
      ! states(i,j,1) = uh ! defined locally
      ! uh(i,j) = uh ! defined locally
      ! vh(i,j) = vh
      ! wh(i,j) = 0.0_rp
      !$acc parallel loop collapse(2) default(present)  private(u1,u2,v1,v2,u_mag,v_mag,uh,vh,tauw) async(1)
      do j = 0,n(2)
      ...
      ! call exchange_data(u,v,w,bcu,bcv)
    end select
